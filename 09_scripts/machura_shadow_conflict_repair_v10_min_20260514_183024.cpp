...
static Hyper build_hyper(const Graph& g) {
    ...
    for (int e = 0; e < (int)h.red.size(); e++) {
        uint64_t m = h.red[e];
        for (int v = 0; v < h.n; v++) {
            if ((m >> v) & 1ULL) h.redInc[v].push_back(e);
        }
    }
    ...
    for (int e = 0; e < (int)h.blue.size(); e++) {
        uint64_t m = h.blue[e];
        for (int v = 0; v < h.n; v++) {
            if ((m >> v) & 1ULL) h.blueInc[v].push_back(e);
        }
    }
...
static void vertices_from_mask(uint64_t m, vector<int>& out) {
    out.clear();
    for (int v = 0; v < 62; v++) {
        if ((m >> v) & 1ULL) out.push_back(v);
    }
}
...
static Best search_one(const Hyper& h, const Params& p, int seed) {
    ...
    vector<int> redActive;
    vector<int> blueActive;
    vector<int> ones, zeros;
    vector<int> conflictVertices;
    ...
    for (int r = 0; r < p.restarts; r++) {
        ...
        for (int step = 0; step < p.steps; step++) {
            ...
            for (int e = 0; e < (int)s.rc.size(); e++) if (s.rc[e] == 4) redActive.push_back(e);
            for (int e = 0; e < (int)s.bc.size(); e++) if (s.bc[e] == 4) blueActive.push_back(e);
            ...
            bool useRed = false;
            if (!redActive.empty() && !blueActive.empty()) {
                useRed = ((rng() & 1ULL) == 0);
            } else if (!redActive.empty()) {
                useRed = true;
            } else if (!blueActive.empty()) {
                useRed = false;
            } else {
                useRed = true;
            }

            if (useRed && !redActive.empty()) {
                int e = redActive[(int)(rng() % redActive.size())];
                vertices_from_mask(h.red[e], conflictVertices);

                for (int one : conflictVertices) {
                    if (!s.x[one]) continue;

                    for (int zero : zeros) {
                        Delta d = swap_delta(h, s, one, zero, p, rm, bm, rs, bs);

                        bool isTabu = (step < tabu[one]) || (step < tabu[zero]);
            ...
            if (!useRed && !blueActive.empty()) {
                int e = blueActive[(int)(rng() % blueActive.size())];
                vertices_from_mask(h.blue[e], conflictVertices);

                for (int zero : conflictVertices) {
                    if (s.x[zero]) continue;
            ...
            if (shake) {
                ...
                for (int z = 0; z < swaps; z++) {
                    ...
                    int one = ones[(int)(rng() % ones.size())];
                    int zero = zeros[(int)(rng() % zeros.size())];

                    Delta d = swap_delta(h, s, one, zero, p, rm, bm, rs, bs);
                    apply_swap(h, s, one, zero, d);

                    tabu[one] = step + p.tabu + (int)(rng() % 5);
                    tabu[zero] = step + p.tabu + (int)(rng() % 5);
                }
                noImprove = 0;
            } else {
                int pick = (int)(rng() % cand.size());
                int one = cand[pick].first;
                int zero = cand[pick].second;
                Delta d = candD[pick];
...
int main(int argc, char** argv) {
    try {
        ...
        p.seed = argi(argc, argv, "--seed", 460055);
        ...
        string resultsPath = outdir + "\\RESULTS_SHADOW_CONFLICT_REPAIR_V10_MIN.tsv";
        string summaryPath = outdir + "\\SUMMARY_SHADOW_CONFLICT_REPAIR_V10_MIN.txt";
        ...
        sum << "MACHURA SHADOW CONFLICT-REPAIR HEURISTIC V10 MIN\n";
        sum << "================================================\n\n";
        sum << "INPUT: " << input << "\n";
        ...
        sum << "ELAPSED_SEC: " << sec << "\n\n";
        sum << "METHOD:\n";
        sum << "Conflict-directed repair over active shadow conflicts.\n";
        sum << "A move is chosen from swaps that break a selected active red or blue K4 shadow.\n";
        sum << "Search preserves degree exactly.\n";
